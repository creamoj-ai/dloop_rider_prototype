...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'riders_read_own_fee_audit' AND tablename = 'fee_audit'
  ) THEN
    CREATE POLICY "riders_read_own_fee_audit" ON public.fee_audit
